...
import { FaEnvelope, FaPhone, FaBriefcase, FaClock, FaUserCheck } from 'react-icons/fa';
...
const Agents = () => {
    ...
    const [isUpdateFormOpen, setIsUpdateFormOpen] = useState(false); // For update form
    const [updateData, setUpdateData] = useState({}); // To handle form input changes
    ...
    const handleUpdateClick = () => {
        ...
        setUpdateData(selectedAgent); // Pre-fill form with selected agent's data
    };
    ...
    const handleUpdateSubmit = async (e) => {
        e.preventDefault()
        try {
            const response = await fetch(`${BaseUrl}/api/updateagents/${selectedAgent._id}`, {
                method: 'PUT',
                headers: {
    ...
    return (
        <div className="container p-4 sm:p-6 md:p-8">
            ...
            <div className="flex justify-end">
                <button onClick={() => navigate('/add_agent')} className="bg-blue-500 text-white px-4 py-2 rounded-full hover:bg-blue-600 transition duration-300">
                    + Add Agent
                </button>
            </div>
                            ...
                            {agentsData.map((agent) => (
                                <tr key={agent._id} className="border-b hover:bg-gray-100 cursor-pointer" onClick={() => handleRowClick(agent)}>
                                    <td className="px-4 py-2 flex items-center">
                                        <img src={agent.image} alt={agent.name} className="w-8 h-8 rounded-full mr-2 bg-cover" />
                                        {agent.name}
                                    </td>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                    <div className="bg-white rounded-lg border border-black shadow p-6 w-full max-w-sm">
                        <div className="mt-4 flex gap-2 items-center">
                            <button onClick={() => handleDeleteAgent(selectedAgent._id)} className="bg-red-500 text-white px-4 py-2 rounded-full hover:bg-red-600 transition duration-300">
                                Delete Agent
                            </button>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-6 bg-[#E8F5FE] p-4">
                            ...
                            <div className="flex items-center">
                                ...
                                <div>
                                    ...
                                    <p className="text-[12px]">{selectedAgent.status ? 'Active' : 'InActive'}</p>
                                </div>
                            </div>
                        </div>
                        <div>
