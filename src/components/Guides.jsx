...
const Guides = () => {
  ...
  useEffect(() => {
    fetch(`${BaseUrl}/api/getguides`)
      ...
      .then((data) => {
        ...
        setGuidesData(data.guide);
        setFilteredGuides(data.guide);
      })
  ...
  const handleDeleteGuide = async () => {
    try {
      const response = await fetch(
        `${BaseUrl}/api/deleteaguides/${selectedGuide._id}`,
        {
          method: "DELETE",
        }
      );
      ...
      if (response.status === 200) {
        setGuidesData((prevData) =>
          prevData.filter((guide) => guide._id !== selectedGuide._id)
        );
        closeModal();
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${BaseUrl}/api/updateguides/${selectedGuide._id}`,
        {
          method: "PUT",
          headers: {
          ...
          <table className="w-full">
            ...
            <tbody>
              {filteredGuides.length === 0 ? (
                ...
                filteredGuides?.map((guides) => (
                  <tr
                    key={guides._id}
                    className="border-b hover:bg-gray-100 cursor-pointer"
                    onClick={() => handleRowClick(guides)}
                  >
            ...
            <div className="flex flex-col items-center mb-4">
              ...
              <div className="flex mt-2">
                <button className="mr-2 p-2 bg-blue-100 rounded-full">
                  <a href="mailto:${selectedGuide.email}">
                    {" "}
            ...
            <div className="grid grid-cols-2 gap-4 mb-6 bg-[#E8F5FE] p-4">
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-[12px]">
                    {selectedGuide.status ? "Active" : "Inactive"}
                  </p>
                </div>
              </div>
            </div>
